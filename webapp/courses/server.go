...
type BlueprintAddButton interface {
	// Returns a partial component for rendering the add button, parameterized by language.
...
type Page interface {
	// Returns the page view component with injected main content, parameterized by language, title, search query, and user ID.
	...
	// Returns the name of the search parameter used in queries (e.g., for extracting search terms from requests).
	SearchParam() string
}
