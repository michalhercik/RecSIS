...
class Result:
    ...
    recommended_true_positive: list[list[bool]]
    recommended_in_degree_plan: list[list[bool]]
    expected: list[str]
    ...
    def __init__(
        ...
        soident: str = None,
        sobor: str = None,
        degree_plan: str = None,
        year_of_study: int = None,
        type: str = None,
        finished: list[str] = None,
        finished_in_degree_plan: list[bool] = None,
        recommended: list[list[str]] = None,
        recommended_in_degree_plan: list[list[bool]] = None,
        expected: list[str] = None,
        expected_in_degree_plan: list[bool] = None,
    ):
        self.soident = soident
        self.sobor = sobor
        self.degree_plan = degree_plan
    ...
    def generate_masks(self, dp_courses):
        self.recommended_true_positive = [
            [course in self.expected for course in group] for group in self.recommended
        ]
        # self.recommended_true_positive = [
        #     code in self.expected for code in self.recommended
        # ]

        self.expected_false_negative = [
            code not in self.recommended for code in self.expected
        ]

        dp = set(dp_courses)
        self.finished_in_degree_plan = [
            True if i in dp else False for i in self.finished
        ]
        self.recommended_in_degree_plan = [
            [course in dp for course in group] for group in self.recommended
        ]
        self.expected_in_degree_plan = [
            True if i in dp else False for i in self.expected
        ]
